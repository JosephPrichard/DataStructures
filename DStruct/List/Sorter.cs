...
{
    private readonly Func<E, E, bool> _doCompare;
    ...
    private int Partition(IList<E> arr, int left, int right)
    {
        ...
        while (left <= right)
        {
            if (_doCompare(arr[left], arr[pivot]))
            {
                i++;
                Swap(arr, i, left);
            }
    ...
    private void Merge(E[] arr, int fi, int mi, int li)
    {
        ...
        System.Array.Copy(arr, fi, left, 0, mi + 1 - fi);
        var right = new E[li - mi];
        System.Array.Copy(arr, mi + 1, right, 0, li - mi);
        var l = 0;
        var r = 0;
        while (l < left.Length && r < right.Length)
        {
            if (_doCompare(left[l], right[r]))
            {
                arr[fi] = left[l];
                l++;
            }
        ...
        if (l >= left.Length)
            System.Array.Copy(right, r, arr, fi, right.Length - l);
        else if (r >= right.Length) System.Array.Copy(left, l, arr, fi, left.Length - l);
    }
    ...
    private Node<E> Merge(Node<E> left, Node<E> right)
    {
        ...
        if (_doCompare(left.Data, right.Data))
        {
